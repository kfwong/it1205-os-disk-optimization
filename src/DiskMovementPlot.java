...
public class DiskMovementPlot extends JPanel {
	...
	private static final long serialVersionUID = -3746967271051437235L;
	 ...
	 *            the frame title.
	 */
	public DiskMovementPlot(DiskOptimizationAlgorithm doa,
			boolean isPlottingFCFS, boolean isPlottingSSTF,
			boolean isPlottingSCAN, boolean isPlottingLOOK,
			boolean isPlottingCSCAN, boolean isPlottingCLOOK) {
		this.doa = doa;
	...
	private XYDataset createDataset() {
		...
		if (isPlottingFCFS)
			dataset.addSeries(createSeries("FCFS", doa.fcfs()));
		if (isPlottingSSTF)
			dataset.addSeries(createSeries("SSTF", doa.sstf()));
		if (isPlottingSCAN)
			dataset.addSeries(createSeries("SCAN", doa.scan()));
		if (isPlottingLOOK)
			dataset.addSeries(createSeries("LOOK", doa.look()));
		if (isPlottingCSCAN)
			dataset.addSeries(createSeries("CSCAN", doa.cscan()));
		if (isPlottingCLOOK)
			dataset.addSeries(createSeries("CLOOK", doa.clook()));
	...
	private JFreeChart createChart(final XYDataset dataset) {
		...
		final JFreeChart chart = ChartFactory.createXYLineChart(
				"Disk Optimisation", // chart title
				"Time Taken", // x axis label
				"Cylinder Position", // y axis label
				dataset, // data
